...
for n in range(620):
    u = 'http://iqiyi.cdn9-okzy.com/20200115/4955_005e9916/1000k/hls/7d236ed02aa{}.ts'.format(
        '0' * (6 - len(str(n))) + str(n))
    urls.append(u)
...
def run(url):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            l = l + r.content
            # 打印访问的状态码是否为200
            with open("D:\\film\\乡村爱情12\\a\\{}.ts".format(urls.index(url)+20), 'wb') as f:  # 以二进制写的方式将r的二进制内容写入path
                f.write(r.content)
                f.close()
