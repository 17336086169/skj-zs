...
for n in range(2200):
    if len(str(n)) < 3:
        u = 'https://haoa.haozuida.com/20180415/0CTAPRlT/800kb/hls/JRguTUCI4019{}.ts'.format(
            '0' * (3 - len(str(n))) + str(n))
        urls.append(u)
    else:
        u = 'https://haoa.haozuida.com/20180415/0CTAPRlT/800kb/hls/JRguTUCI4019{}.ts'.format(str(n))
        urls.append(u)
...
def run(url):
    try:
        ...
        if r.status_code == 200:
            ...
            # l = l + r.content
            # 打印访问的状态码是否为200
            with open("D:\\film\\复仇者联盟2\\复仇者联盟2\\{}.mp4".format(urls.index(url)), 'wb') as f:  # 以二进制写的方式将r的二进制内容写入path
                f.write(r.content)
                f.close()
                # print('第', urls.index(url), "个文件保存成功！")
                print('下载进度: 已下载', urls.index(url) / len(urls), '%')
        else:
            print('下载结束或者文件保存失败！')
    except:
        print('请求出错！')
